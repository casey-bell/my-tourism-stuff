...
def raw_sample():
    ...
    # - Quarter strings requiring normalisation (trimmed to avoid spurious
    #   whitespace)
    return pd.DataFrame(
        {
...
def test_no_missing_in_key_fields_after_cleaning(raw_sample):
    ...
    completeness = cleaned[other_fields].notna().all().all()
    assert completeness
...
def test_deduplication_and_row_integrity(raw_sample):
    ...
    duplicated = pd.concat(
        [raw_sample, raw_sample.iloc[[0]]], ignore_index=True
    )
    cleaned = clean(duplicated)
    # Expect duplicates removed or consolidated
