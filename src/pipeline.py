...
def make_visitors_by_quarter(
    ...
    This now explicitly runs the cleaning step using:
      - src.data.clean.clean
      - src.data.clean.DEFAULT_INPUT (as default input)
      - src.data.clean.DEFAULT_OUTPUT (as default cleaned output)

    Parameters
    ----------
    input_path : Path | str
        Path to the raw input used by the cleaning step (defaults to DEFAULT_INPUT).
    cleaned_output_path : Path | str
        Path to the cleaned output generated by the cleaning step (defaults to DEFAULT_OUTPUT).
    """
    print("🔄 Starting visitors_by_quarter generation...")

    # 0) Ensure the cleaned dataset is (re)generated using the canonical clean() entrypoint
    print(f"🧼 Cleaning data from {input_path} -> {cleaned_output_path} ...")
